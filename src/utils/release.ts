import type { ReleaseManifest, OSType, DownloadOption, PlatformDownloads } from '../types/release';
...
export async function fetchReleaseManifest(
  ...
  const url = manifestUrl || 'https://downloads.lifevault.sabrlabs.co.uk/latest.json';
  
  try {
    const response = await fetch(url);
    
    if (!response.ok) {
      console.warn(`Failed to fetch release manifest: ${response.status} ${response.statusText}`);
      return null;
    }
